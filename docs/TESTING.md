...
571 tests. They are not unit tests of functions. Almost every one of them is a guard against
...
567 passed, 4 skipped in 3.51s
```

On a fresh checkout the same command gives `499 passed, 7 skipped` — see
[What a fresh checkout runs](#what-a-fresh-checkout-runs).
...
project's own `docs-init` detector did exactly that. It is wrong by 571.
...
`ngtui/`. That is why the suite must be run from `ngtui/` and not from the root.
...
uv run --with pytest python -m pytest -q -k "window"        # 62 passed, 509 deselected
```
...
### What a fresh checkout runs

The author's box collects 571. A fresh checkout collects 506 and runs 499. The difference is
`test_theme.py` (12) and `test_theme_roles.py` (55), which both open with

```python
pytest.importorskip("textual", reason="the terminal app it themed is retired")
```

`ngtui/pyproject.toml:6` declares `dependencies = []`. Textual is not one of them — it was,
while the terminal app existed. Those two files cover the palette→`textual.theme.Theme`
mapping that fed it, so they skip at module level rather than erroring at collection.
Everything the shipped product reads out of `theme.py` (`raw_tokens()`, `omarchy_style()`)
needs no Textual and is covered by `test_theme_sync.py`, which runs everywhere.

On the author's box they *do* run, because `ngtui/.venv` still contains a leftover
`textual 8.2.7` that `uv.lock` does not mention. That is the whole 65-test gap. If you want
...
`test_first_install.py:291` now holds that line:
`test_no_package_module_imports_a_dependency_this_package_does_not_declare` walks the package
with `ast` and fails on a module-level import of anything `pyproject.toml` does not declare. A
lazy import inside a function is allowed and is what the `Theme` builders use now — the import
surface is the part that decides whether a module can be loaded at all.

One more skip appears on a checkout that is not a git working tree
(`test_deploy_completeness.py:390` reads the git index for file modes) — see the skip list
below.
...
| `test_port02_preview_seam.py` | 4 | `preview_change` calls the *same function objects* as the live `nightguard_ctl`, so the preview cannot drift from the signer. 3 of the 4 skip without a live instance — see below. |
...
| `test_lineedit.py` | 14 | Every per-field edit is a line edit on the sanctioned text, never a YAML re-emit. Two assertions per test: the edit re-parses through the live `ngcommon.yaml_load` to the expected value, and every other line is byte-identical. Includes the empty-list round trip: inline `[]` → one item → back to inline `[]`, plus "removing one of several leaves the list a list". |
| **The staged-edit vocabulary** | | |
| `test_proposal.py` | 24 | The editable key set is closed; a list entry is validated against a whitelist rather than escaped; a newline cannot smuggle a second entry; a failing op rolls the whole batch back; no ops at all returns the config byte for byte. |
| `test_catalog.py` | 11 | The picker's list: only a process the panel actually read is `verified`; a webapp is never offered as an application; the floor never appears; an entry the config already names always appears even when nothing on this machine answers to it. |
| **The panel payload** | | |
...
| `test_parity.py` | 5 | The terminal editor's field table is a subset of `proposal.EDITABLE`; every editable key is one the signer's `FIELD_TABLE` classifies; the one field left behind (`blocking.browser_extension.extension_id`) is left behind because the signer has no direction for it, so it would price as free and be written anyway. |
| **The deploy** | | |
...
| `test_first_install.py` | 13 | No install artefact carries the author's username; the sudoers owner is a substituted placeholder; nothing shipped grants a grace window and `CLAUDE.md` still says the bypass does not exist; and no module under `ngtui/ngtui/` imports at module level a distribution `pyproject.toml` does not declare (`:291`). |
| `test_installer_idempotent.py` | 4 | A re-run detects its own marker and does not duplicate; the file is backed up to `<name>.bak.<epoch>` before the first edit; everything outside the managed block comes out byte-identical, comments included. |
...
| **The QML guards** (they read the files as text; nothing executes QML) | | |
...
| `test_qml_composition.py` | 6 | The dial is drawn by exactly one file, the duration wording lives in exactly one file, the window lands on the glance, an unread defence is never drawn as switched off, the defence switches refuse to move before the reading arrives, and `open()` rejects a view name the window does not have. |
| `test_deploy_completeness.py` (3 of its 18) | — | **Type resolution** (`:147`): every `Foo {` resolves to a plugin file, a shell-kit component, an inline component or a listed import. **Layout measurement** (`:265`): no `Toggle` carries a `description`, and every `wrapMode: Text.WordWrap` has an explicit `width` above it. **Privilege** (`:226`): no QML names `pkexec`, `sudo`, `nightguard_ctl` or `/usr/bin/python3` outside a comment. |
...
| **The theme** | | |
| `test_theme_sync.py` | 143 | Every installed omarchy theme maps to a complete panel palette, with provenance: each role must be a colour the theme itself declares. Parametrised over the themes found on disk. This is the file that covers what the shipped panel reads. |
| `test_theme_roles.py` | 55 | The same sweep for the Textual role mapping, plus a vacuity guard asserting both sweeps agree on what "installed" means. Skipped without Textual. |
| `test_theme.py` | 12 | `colors.toml`/`alacritty.toml` → `Theme` + `ThemeWatch`, against tmp-dir fixtures. Skipped without Textual. |
| | **571** | |
...
what the function does. Several carry the measured number from the day it broke.
...
of the tree so nothing in the working tree was left armed):

| Guard | Arm it with | Message |
| --- | --- | --- |
| `test_deploy_completeness.py::test_every_component_a_qml_file_instantiates_actually_resolves` | append `Item {` / `  Bogus {` / `  }` / `}` to `NightguardGlance.qml` | `NightguardGlance.qml:NNN instantiates Bogus, which is neither a file in the plugin, a component in the shell kit, an inline component of this file, nor a listed imported type — it resolves to nothing` |
...
The injected component matters more than it looks: the scan's regex is
`^\s*([A-Z][A-Za-z0-9]*)\s*\{\s*$`, so `Item { Bogus {` on one line slips past and the test
stays green. The first attempt at this control did exactly that. Check your control actually
armed the defect before concluding the guard is weak.
...
- **`test_no_fixed_hex.py:76` — `test_the_scan_catches_an_injected_hex`.** It appends
  ...
  evidence."* The same file's first test also asserts that some plugin source references
  `Color.` at all, so a scan pointed at the wrong tree is caught.
- **`test_port03_no_crypto.py:53` — `_iter_py_files`.** Asserts the scan reached at least one
  ...
  `theme.py`, `panel.py`, `proposal.py`, `catalog.py`). Message: `PORT-03 SCAN IS VACUOUS`.
...
- **`test_deploy_completeness.py:82` — `test_the_watchdog_really_does_import_the_blocker`.**
  Guards the transitive-import check above it from passing vacuously if the import is dropped
  while the enforcement call stays.
- **`conftest.py:446`** — `mutate_theme` asserts the section it was asked to rewrite exists,
  so a restyle test cannot pass by asserting on a change that never happened.

Three of the QML scans have **no** such guard:
`test_every_component_a_qml_file_instantiates_actually_resolves`,
...
nothing. `test_weekly_tokens_source.py:77` has a milder version — `if "NightguardWriter {" not
in text: continue` skips a file silently rather than asserting the instantiation is still
there.
...
hand and needs no root. It does two things no unit test can: checks each policy file is
present, root-owned, not writable by the owner and byte-identical to what the watchdog would
write right now; and starts each Gecko browser headless against a throwaway profile with the
policy engine's debug logging on, asserting the path it reports reading is the `/etc` one.
Exit 0 = locked, 1 = at least one check failed. `test_watchdog_enforcement.py` covers the
*policy body* the watchdog would write — not whether the browser reads it. That distinction is
the whole reason the script exists: until 2026-09-12 the Gecko half was a staged file at
`/var/lib/nightguard/policies/` that no code read, no deploy installed and no watchdog
restored, while the add-on sat switched off in Zen.

**No coverage measurement, no CI.** `ngtui/pyproject.toml` configures no coverage threshold
and no coverage tool. There is no `.github/`, no `.gitlab-ci.yml`, no CI of any kind. Nothing
runs this suite except a person typing the command.

### The skipped tests

On the author's box, four:
...
On a fresh checkout, three more:

```
SKIPPED [1] tests/test_theme.py:22:       the terminal app it themed is retired
SKIPPED [1] tests/test_theme_roles.py:45: the terminal app it themed is retired
SKIPPED [1] tests/test_deploy_completeness.py:390: not a git checkout — there is no index to read modes from
```

**`test_parity.py::test_the_frozen_copy_still_matches_the_living_table`** skips because
`ngtui/ngtui/widgets/edit.py` no longer exists. Permanent and by design: the terminal app was
deleted, and the frozen `TERMINAL_EDITOR_FIELDS` tuple at `test_parity.py:33-47` is now the
historical record the parity claim rests on. The other four tests in that file still run
against it.

**The two theme skips** are the Textual `importorskip` described above.

**The `test_deploy_completeness.py:390` skip** only appears outside a git working tree — the
test reads `git ls-files -s` to check file modes in the index, because `core.filemode=false`
here means a chmod on disk never reaches git.

**The three `test_port02_preview_seam.py` skips** happen because `backend.sanctioned_text()`
is empty, and it is empty because `conftest.py:26-28` deliberately points `NIGHTGUARD_DIR` at
`<repo>/.pytest-no-instance`:

> A path that deliberately does not exist. Every test that touches an instance points
> `NIGHTGUARD_DIR` at its own `tmp_path`; this default exists only so that a test which
> forgets to fails loudly instead of reading, or writing, the real instance on the machine
> running the suite.

That is the right default. But it means these three never run anywhere, and one of them is
broken. Pointed at the live instance:
...
`backend.preview_change` now passes `edit_window=` and `now_minutes=` to `quota_decide`, and
the test's stand-in does not accept them. The skip has been hiding a broken test since the
edit window landed. Anything that changes the `quota_decide` signature should re-run this file
with `NIGHTGUARD_DIR` pointed at a real instance, or the seam it guards is unguarded.
...
and the seam that puts the gate in front of the user before he authenticates, are two
different promises. There are no subdirectories and no test classes.
...
Not `test_kill_decision` or `test_config_owner`. The name states what must be true, so a line
in the failure summary is already the bug report. `test_..._not_...` and
...
function under test does. If the defect was measured, put the number in. If it is a class of
bug rather than a single incident, say what the class costs. Compare:
...
and the message must say the scan was vacuous. Three existing scans lack this; do not add a
fourth. `test_port03_no_crypto.py:53` is the pattern to copy.
...
Every seam that exists for testing, and what stops it being a bypass.
...
| `NIGHTGUARD_NTP_SERVER` | `scripts/linux/guard.py:35` | Which SNTP server is queried. | No, and it is not a bypass: the guard still requires an answer and still checks drift. |
| `NG_HYPR_CONF`, `NG_ICON_BASE`, `NG_APPLICATIONS_DIR`, `NG_BIN_DIR`, `NG_SKIP_RELOAD` | `packaging/omarchy/install.sh`, used at `test_installer_idempotent.py:35-42` | Redirect every write of the author-facing installer into a temp dir; suppress the prereq check and the Hyprland reload. | Irrelevant — that installer is unprivileged and touches no signed state. |
| `OMARCHY_PATH` | `test_deploy_completeness.py:165` | Where the shell kit's QML components are looked up for the type-resolution scan. | Test-only; reads nothing privileged. |
...
| `ngtui.theme.OMARCHY_THEME_DIRS` | `conftest.py:370-380` | Which theme directory the loader reads. Monkeypatched, deliberately **not** an env var: `theme.py` has no env seam, and a fixture that set one would leave the loader reading the live desktop theme while looking like it had not. | Same. |
| `read_key_explodes` fixture | `conftest.py:154` | Makes `read_key` raise on `ng`, `guard.ng` and `ctl.ng` so a stray read from any of them is caught on the key-less status path. | A fixture, not a seam into the product. |
...
`ROOT_INSTANCE` is `/var/lib/nightguard` (`ngcommon.py:27`); `LEGACY_INSTANCE` is
`~/.local/share/nightguard` (`:32`). Both count, so the seams stay dead across the migration
between them.

What it protects against is stated at `guard.py:107-113`: the adversary is the user, so any
...
Measured, `cd scripts/linux` then `NIGHTGUARD_DIR=<x> python3 -c 'import ngcommon as ng; print(ng.IS_CANONICAL_INSTANCE)'`:

| `NIGHTGUARD_DIR` | `IS_CANONICAL_INSTANCE` |
| --- | --- |
| `/var/lib/nightguard` | `True` |
| `~/.local/share/nightguard` | `True` |
| `/tmp/fake-instance` | `False` |
| *(unset)* | `True` |

The unset row is the one that matters: the panel, the signer and the watchdog all run with
...
(`nightguard-watchdog.service`, the sudoers rule, the polkit action) is pinned to absolute
paths by root-owned unit files, and none of it reads the user's environment.

## Three things to know before you trust a green run

**1. A green suite is not a green desktop.** The tests run against the working tree; the
desktop runs `~/.local/share/uv/tools/ngtui/` and `/usr/local/lib/nightguard/`, both written
by `deploy.sh`. Measured 2026-09-14: `theme.py` in the installed tool still carried a
module-level `from textual.theme import Theme` (line 39) that the working tree no longer has,
so `__main__._theme_tokens()` (`ngtui/ngtui/__main__.py:318`) and `_style_tokens()` (`:328`),
both wrapped in `except Exception: return {}`, silently returned `{}` and the payload came out
of `THEME_FALLBACK` (`ngtui/ngtui/panel.py:217`):

```
$ cat ~/.local/state/omarchy/current/theme/colors.toml   # accent = "#7aa2f7"  (Tokyo Night)
$ ngtui panel | jq -c .theme
{"background":"#2d353b", ... ,"accent":"#7fbbb3", ...}   # Everforest — the fallback
```

No test can catch that class of drift, because every test imports from the tree. `deploy.sh`
is the only thing that closes it. `test_deploy_completeness.py` exists to make sure the deploy
at least carries every file; it cannot make you run it.

**2. `ngtui/tests/test_lineedit.py:23-25` still hardcodes the author's absolute path** as its
`NIGHTGUARD_STACK_DIR` fallback. It is dead today only because `conftest.py:19-21` derives the
real one from `__file__` and sets the variable first, so the literal is never reached. It was
live until 2026-09-14, and the effect was worse than an error: a full copy of this repo run
from `/tmp` passed the entire suite while `backend.ctl.__file__` pointed at the author's clone.
Editing `WEEKLY_TOKENS` in the copy changed nothing any test saw. If you move that
`setdefault` out of `conftest.py`, the literal comes back to life.

**3. The suite still carries the retired Textual UI's scaffolding.** Four of `conftest.py`'s
eight fixtures — `probe_app` (`:475`), `mutate_theme` (`:414`), `omarchy_theme_dir` (`:384`)
and `omarchy_theme_pinned_dir` (`:396`), roughly 250 of the file's 579 lines — are referenced
by no test in the suite. They were written for a UI wave that was retired. `probe_app` is also
the only thing in `conftest.py` that imports Textual, and nothing calls it. Do not copy their
shape without checking whether anything still uses them.
