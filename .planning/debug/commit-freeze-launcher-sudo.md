...
- timestamp: 2026-07-05 — **Even `sudo -S` (in-window password) FROZE at "Committing…".** Root cause is `pam_fprintd` (fingerprint) being the FIRST + `sufficient` auth in /etc/pam.d/sudo, with prints enrolled (Elan sensor). Proof: `printf pw | sudo -S -k -p '' true` → exit 124 (hang); a PTY-fed password also shows "Place your right index finger on the fingerprint reader" and blocks (password never consumed). So NO password-piping design can work here — the only auth is the fingerprint scan (or the sensor failing over to a tty password).
- timestamp: 2026-07-05 — **Fix = fingerprint touch on a PTY, in-window.** `sudo -k … commit` on a pty (`_run_sudo_pty`) activates the sensor; the user touches it from the dialog. Verified in the real floated window (wtype-drove e/space/c/y): the sensor prompt appeared and the flow executed. 84 tests pass (incl. real PTY capture/rc/cancel-130/timeout-124/stream). NB: driving a live `y` in a test triggered a real auth + spent a token; config verified intact (curfew.enabled still true).
