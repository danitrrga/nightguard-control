...
Next: Phase 8 (native-app blocker) — folds the Hyprland window-class kill into the root watchdog tick. MAIN RISK: the root tick reaching the user's Hyprland socket /run/user/1000/hypr/$HIS/.socket.sock (discover XDG_RUNTIME_DIR + instance sig for uid 1000). Start with `/gsd-discuss-phase 8` → `/gsd-plan-phase 8`.
