...
### Phase 7.1: Curfew Hook (Linux) *(gap-closure)*
**Goal**: Make curfew actually enforce on Linux. `guard.py` already returns the correct verdict (`deny` during curfew, `allow` outside / in grace) but **no Claude Code hook fires it**, so sessions are never blocked. Wire the guard verdict into the Claude Code hook contract — the Linux equivalent of the retired Windows `nightguard_adapter.ps1`.
**Depends on**: Phase 7 (root watchdog; user-context guard now runs in soft mode)
**Requirements**: CURF-01
**Discovered**: 2026-06-22, during Phase 7 execution — at 22:54 (in curfew) `guard.py` returned `{"decision":"deny","reason":"curfew"}` (exit 1) yet nothing blocked, because `~/.claude/hooks` has no nightguard hook and nothing references `guard.py` (matches the "LifeOS hooks not wired on Linux" migration note).
**Success Criteria** (what must be TRUE):
  1. During curfew, a Claude Code session/prompt is blocked with the Spanish-butler deny message (`guard.py` exit 1 → hook denies).
  2. Outside curfew — and during an active daily grace window — sessions are allowed.
  3. The hook reads the canonical LifeOS config/guard (no `~/.claude` drift; mirrors WIRE-01) and runs `guard.py` in user soft-mode (integrity stays the root watchdog's job; the hook does not need the key).
**Notes for planning (2026-06-23)**: reference the retired Windows adapter `LifeOS/.agents/hooks/nightguard_adapter.ps1` for the verdict→hook-contract mapping; determine the right Claude Code hook event(s) on this runtime (SessionStart / UserPromptSubmit) and the deny/exit contract; decide where the hook lives (`~/.claude/hooks` is regenerated by the LifeOS shared agent-config system — wire it in LifeOS `config/`, not by hand).
**Plans**: 3 plans
...
### Phase 9: StayFree Desktop *(was "ActivityWatch" — re-scoped 2026-06-23)*
**Goal**: StayFree desktop (AUR `stayfree-desktop`) becomes the primary app+website blocker and the screen-time/analytics source on Linux, supervised by the root watchdog so it cannot be quit to escape — with Phase 8's root kill retained as a root-owned backstop floor for the critical native apps.
**Depends on**: Phase 7 (root watchdog) + Phase 8 (root native kill, demoted to backstop)
**Requirements**: TRAK-01, TRAK-02 (screen-time/analytics now sourced from StayFree, not ActivityWatch)
**Gating spike (run FIRST)**: Prove StayFree desktop actually *blocks* (not just tracks) under Hyprland/Wayland. If it cannot block on Wayland → fall back to StayFree-analytics-only and Phase 8 stays primary.
**Success Criteria** (what must be TRUE):
  1. `stayfree-desktop` is installed and configured as the primary app+website blocker; its analytics replace the parked ActivityWatch idea (ActivityWatch retired).
  2. The root watchdog `tick()` keeps StayFree running — if the user kills it, it is respawned into the user session (runuser bridge), so it cannot be escaped by quitting.
...
**Plans**: 3 plans (spike-first, branch-on-result)
  - [x] 09-01-PLAN.md — Wave 0 D-09 gating spike (install + observe blocks-vs-tracks under Hyprland) + record PASS/FAIL/PARTIAL verdict + rewrite stale TRAK-01/02 to StayFree terms (TRAK-01, TRAK-02) — **unconditional**
  ...
  - [x] 09-03-PLAN.md — **IF spike FAIL/PARTIAL** (predicted): record StayFree = analytics-only, keep-alive moot/not-built, re-open CONTEXT per D-09 (TRAK-01, TRAK-02)
...
v2.0 critical path: 6 → 7 → 8 → 10 (8 folds into the P7 watchdog; 10 is the TUI thin-client). Phase 9 (StayFree Desktop, re-scoped from ActivityWatch 2026-06-23) depends on 7+8 and is gated on a Wayland-blocking spike; ActivityWatch retired.
...
| 10. Linux App (omarchy TUI) | 1/3 | In Progress|  |
