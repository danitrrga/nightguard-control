...
**Plans**: 3 plans
  - [ ] 07.1-01-PLAN.md — Wave-0 runtime probe: capture the UserPromptSubmit `prompt` field + record the substring-vs-leading-token matcher decision (Q1)
  - [ ] 07.1-02-PLAN.md — Port `nightguard_adapter.py` (verdict→exit-2 + allowlist + butler messages, fail-closed) + add `/plan` to `config.yaml` via the signed control-CLI (CURF-01 core)
